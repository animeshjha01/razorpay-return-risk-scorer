...
def generate_data(n=5000, seed=42, test_size=0.2):
    # Use an explicit local RandomState for mathematical reproducibility
    rng = np.random.RandomState(seed)
    
    # Deterministic base fields
    order_ids = [f"order_{str(i+1).zfill(7)}" for i in range(n)]
    payment_ids = [f"pay_{str(i+1).zfill(7)}" for i in range(n)]
    
    # Deterministic dates over a 30-day window anchored to a fixed UTC datetime
    # SYNTHETIC_START = fixed UTC datetime
    end_date = datetime(2026, 1, 1, 12, 0, 0)
    start_date = end_date - timedelta(days=30)
    total_seconds = int((end_date - start_date).total_seconds())
    created_at = [start_date + timedelta(seconds=int(rng.randint(0, total_seconds))) for _ in range(n)]
    
    # Amount INR (skewed log-normal)
    amount_inr = rng.lognormal(mean=6.5, sigma=1.0, size=n).round(2)
    amount_inr = np.clip(amount_inr, 50, 50000)
    
    # Method
    methods = ['upi', 'card', 'netbanking', 'wallet', 'cod']
    method = rng.choice(methods, p=[0.4, 0.2, 0.1, 0.05, 0.25], size=n)
    
    # Category
    categories = ['electronics', 'apparel', 'grocery', 'digital', 'home']
    category = rng.choice(categories, p=[0.2, 0.3, 0.2, 0.15, 0.15], size=n)
    
    # Customer history
    is_new_customer = rng.choice([0, 1], p=[0.7, 0.3], size=n)
    past_orders = np.where(is_new_customer, 0, rng.poisson(lam=5, size=n))
    
    # past_return_rate - forced 0 for new customers
    past_return_rate = np.where(is_new_customer, 0.0, rng.beta(a=1, b=5, size=n))
    ...
    delivery_distance_km = rng.exponential(scale=15, size=n).round(1)
    checkout_time_sec = rng.lognormal(mean=4.0, sigma=0.8, size=n).round(1)
    ...
    method[digital_cod_mask] = rng.choice(non_cod_methods, size=digital_cod_mask.sum(), p=non_cod_probs)
    ...
    # Labels
    df['returned_or_chargeback'] = rng.binomial(1, prob)
    
    # Split - note: train_test_split is fully deterministic when given random_state
